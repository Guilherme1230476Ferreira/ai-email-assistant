...
use crate::{models::domain::Role, AppError};
...
impl RoleRepository {
    ...
    pub async fn create_role(&self, name: &str) -> Result<Role, AppError> {
        sqlx::query_as!(Role, "INSERT INTO roles (name) VALUES ($1) RETURNING *", name)
            .fetch_one(&*self.pool)
            .await
            .map_err(|e| {
                if let sqlx::Error::Database(db_err) = &e {
                    if db_err.is_unique_violation() {
                        return AppError::new(StatusCode::CONFLICT, "Role with this name already exists");
                    }
                }
                e.into()
            })
    ...
    pub async fn get_all_roles(&self) -> Result<Vec<Role>, AppError> {
        Ok(
            sqlx::query_as!(Role, "SELECT * FROM roles ORDER BY name")
                .fetch_all(&*self.pool)
                .await?,
        )
    }
