<script lang="ts">
  ...
  import {
    ...
    LogOut, Sparkles, Menu, X, ShieldAlert
  } from '@lucide/svelte';
...
<div class="flex min-h-screen w-full bg-black text-white">
  ...
  <aside class="fixed inset-y-0 left-0 z-40 flex w-60 flex-col border-r border-[var(--color-border)] bg-[var(--color-surface)] transition-transform duration-300 lg:translate-x-0 {sidebarOpen ? 'translate-x-0' : '-translate-x-full'}">
        ...
        {#each nav as item (item.href)}
          ...
          <li>
            <a href={item.href}
              ...
              {/if}
              <item.icon class="h-4 w-4 {active ? 'text-[var(--color-accent)]' : 'text-[var(--color-muted)] group-hover:text-white'}" />
    ...
    <div class="border-t border-[var(--color-border)] p-3">
      {#if data.user}
        <div class="flex items-center gap-3 rounded-md px-2 py-2">
          ...
          <!-- Logout: POST to /logout server action — clears the HttpOnly cookie -->
