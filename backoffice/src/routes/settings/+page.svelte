<script lang="ts">
  ...
  async function handleSave() {
    ...
    isSaving = false;
    if (error) {
      saveResult = { ok: false, message: error };
      return;
    }
    apiKey = '';
    saveResult = { ok: true, message: 'Settings saved successfully.' };
    await invalidateAll();
  }
  ...
  async function handleTest() {
    isTesting = true;
    testResult = null;
    const { data: resp, error } = await api.verifySettings({
      llm_model: model,
      llm_base_url: baseUrl,
      ...(apiKey ? { llm_api_key: apiKey } : {})
    });
    ...
    testResult = { ok: true, message: resp?.message ?? 'Connection successful!' };
  }
</script>
...
<div class="mx-auto w-full max-w-3xl space-y-8">
  ...
  <div class="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
    <div class="space-y-4">
      ...
      {#if testResult}
        <div class="flex items-center gap-2 rounded-md border px-4 py-3 text-sm
          {testResult.ok
